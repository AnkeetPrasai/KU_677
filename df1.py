            ...
            if line.startswith("define"):
                ...
                if match:
                    ...
                    if origin1 == "SOURCE" or origin2 == "SOURCE":
                        ...
                        # If operand1 is a variable, use its origin; else use operand1 (constant)
                ...
                # Example with assignment: %secret = call i32 () @SOURCE()
                ...
                if match:
                    ...
                    if called_func == "SOURCE":
                        ...
                        origin = get_origin(argument, data_flow) if argument.startswith('%') else argument
                        print(f"Function SINK called with argument origin: {origin}")
                        if origin == "SOURCE":
                            has_flow = True
                else:
                ...
                # Example: %phiVar = phi i32 [42, %branch2], [%secret, %branch1]
                # Updated regex to handle both variables and constants
                match = re.match(r'(%\w+)\s*=\s*phi\s+\w+\s+\[(%?\w+),\s*%(\w+)\],\s+\[(%?\w+),\s*%(\w+)\]', line)
                if match:
                    dest = match.group(1)
                    source1 = match.group(2)  # Can be a variable (%secret) or constant (42)
                    label1 = match.group(3)
                    source2 = match.group(4)  # Can be a variable (%varF) or constant
                    label2 = match.group(5)
                    
                    print(f"Detected phi instruction: {line}")
                    print(f"Destination: {dest}, Source1: {source1}, Source2: {source2}")
                    
                    # Determine origins
                    origin1 = get_origin(source1, data_flow) if source1.startswith('%') else source1
                    origin2 = get_origin(source2, data_flow) if source2.startswith('%') else source2
